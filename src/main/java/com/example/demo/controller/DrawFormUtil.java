...
import com.example.demo.entity.Grid;
import com.example.demo.entity.UserCell;
...
public class DrawFormUtil {
    ...
    public static List<List<String>> readXlsx(String path, int pageNum) throws Exception {
        ...
        // numSheet++) {
        HSSFSheet xssfSheet = xssfWorkbook.getSheetAt(pageNum);// 获取当前页
        if (xssfSheet == null) {
            // continue;
        } else {
            // 处理当前页，并处理每一行
            for (int rowNum = 0; rowNum < xssfSheet.getLastRowNum(); rowNum++) {
                HSSFRow xssfRow = xssfSheet.getRow(rowNum);// 获取行数据
                ...
                for (int colIX = minColIX; colIX < maxColIX; colIX++) {
                    HSSFCell cell = xssfRow.getCell(colIX);// 获取单元格
                    if (cell == null) {
                        continue;
                    } else {
    ...
    public static void main(String[] args) throws Exception {
        String path = "C:\\Users\\win10\\Desktop\\4－1：专项附加扣除信息电子模版.xls";
        test(path);
    }
